...
class myHeteroscedasticGaussian(Gaussian):
    def __init__(self, Y_metadata, gp_link=None, variance=1., common_variance=1., name='het_Gauss'):
        if gp_link is None:
            gp_link = link_functions.Identity()
        ...
        super(myHeteroscedasticGaussian, self).__init__(gp_link, common_variance*np.ones(Y_metadata['output_index'].shape)*variance, name)
        
        self.common_variance = Param('common_variance', common_variance, Logexp())
        self.link_parameter(self.common_variance)

    def exact_inference_gradients(self, dL_dKdiag,Y_metadata=None):
        return dL_dKdiag[Y_metadata['output_index']]

    def gaussian_variance(self, Y_metadata=None):
        return self.common_variance * self.variance
    
    def update_gradients(self, grad):
        self.common_variance.gradient = grad
        
    def moments_match_ep(self, data_i, tau_i, v_i, Y_metadata_i=None):
        v = self.variance * self.common_variance
        sigma2_hat = 1./(1./v + tau_i)
        mu_hat = sigma2_hat*(data_i/var + v_i)
        sum_var = v + 1./tau_i
        Z_hat = 1./np.sqrt(2.*np.pi*sum_var)*np.exp(-.5*(data_i - v_i/tau_i)**2./sum_var)
        return Z_hat, mu_hat, sigma2_hat
    ...
    def predictive_values(self, mu, var, full_cov=False, Y_metadata=None):
        if Y_metadata == None:
            return mu, var
        _s = self.common_variance * self.variance
        if full_cov:
            if var.ndim == 2:
                var += np.eye(var.shape[0])*_s
            if var.ndim == 3:
    ...
    def predictive_variance(self, mu, sigma, predictive_mean=None):
        return self.variance*self.common_variance + sigma**2
    ...
    def pdf_link(self, link_f, y, Y_metadata=None):
        #Assumes no covariance, exp, sum, log for numerical stability
        return np.exp(np.sum(np.log(stats.norm.pdf(y, link_f, np.sqrt(self.variance*self.common_variance)))))
    
    def logpdf_link(self, link_f, y, Y_metadata=None):
        ln_det_cov = np.log(self.variance*self.common_variance)
        return -(1.0/(2*self.variance*self.common_variance))*((y-link_f)**2) - 0.5*ln_det_cov - 0.5*np.log(2.*np.pi)
    
    def dlogpdf_dlink(self, link_f, y, Y_metadata=None):
        s2_i = 1.0/(self.variance*self.common_variance)
        grad = s2_i*y - s2_i*link_f
        return grad
    ...
    def d2logpdf_dlink2(self, link_f, y, Y_metadata=None):
        N = y.shape[0]
        D = link_f.shape[1]
        hess = -(1.0/(self.variance*self.common_variance))*np.ones((N, D))
        return hess
    
    def dlogpdf_link_dvar(self, link_f, y, Y_metadata=None):
        e = y - link_f
        s_4 = 1.0/((self.variance*self.common_variance)**2)
        dlik_dsigma = -0.5/(self.variance*self.common_variance) + 0.5*s_4*np.square(e)
        return dlik_dsigma
    ...
    def _variance(self, gp):
        return self.variance*self.common_variance
    ...
    def samples(self, gp, Y_metadata=None):
        ...
        #orig_shape = gp.shape
        gp = gp.flatten()
        Ysim = np.array([np.random.normal(self.gp_link.transf(gpj), scale=np.sqrt(self.variance[Y_metadata['output_index'].flatten()]*self.common_variance), size=1) for gpj in gp])
        return Ysim.reshape(orig_shape)
    
    def log_predictive_density(self, y_test, mu_star, var_star, Y_metadata=None):
        v = var_star + self.variance*self.common_variance
        return -0.5*np.log(2*np.pi) -0.5*np.log(v) - 0.5*np.square(y_test - mu_star)/v
    ...
    def variational_expectations(self, Y, m, v, gh_points=None, Y_metadata=None):
        ...
        lik_var = float(self.variance*self.common_variance)
        F = -0.5*np.log(2*np.pi) -0.5*np.log(lik_var) - 0.5*(np.square(Y) + np.square(m) + v - 2*m*Y)/lik_var
        dF_dmu = (Y - m)/lik_var
        dF_dv = np.ones_like(v)*(-0.5/lik_var)
        dF_dtheta = -0.5/lik_var + 0.5*(np.square(Y) + np.square(m) + v - 2*m*Y)/(lik_var**2)
...
class yourHeteroscedasticGaussian(Gaussian):
    def __init__(self, Y_metadata, gp_link=None, variance=1., common_variance=1., name='het_Gauss'):
        if gp_link is None:
            gp_link = link_functions.Identity()
        ...
        super(yourHeteroscedasticGaussian, self).__init__(gp_link, common_variance*np.ones(Y_metadata['output_index'].shape)*variance, name)
